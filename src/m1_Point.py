...
class Point(object):
    ...
    def move_to(self,dx,dy):
        self.distance = ((self.x - (dx))**2 + (self.y - (dy))**2)**.5 + self.distance
        self.x = dx
        self.y = dy
        self.count = self.count + 1

    def move_by(self,dx,dy):
        self.distance = ((self.x - (self.x + dx))**2 + (self.y - (self.y + dy))**2)**.5 + self.distance
        self.x = self.x + dx
        self.y = self.y + dy
        self.count = self.count + 1
    ...
    def get_distance_from_start(self):
        distance = ((self.x - self.p0x)**2 + (self.y - self.p0y)**2)**.5
        return distance
